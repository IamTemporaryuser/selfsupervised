...
class PWSLinear(nn.Module):
    def __init__(self, in_features, out_features, equiv=False, gamma=1e-5, initalpha=False, bias=True):
        super(PWSLinear, self).__init__()
        self.in_features = in_features
        self.out_features = out_features

        self.weight = Parameter(torch.Tensor(out_features, in_features))
        self.alpha = Parameter(torch.Tensor(out_features, 1))
        if bias:
            self.bias = Parameter(torch.Tensor(out_features))
        else:
            self.register_parameter('bias', None)

        self.constval = math.sqrt(2 / float(in_features))

        self.norm_shape = [in_features]

        self.gamma = 1 / float(in_features) * gamma if equiv else gamma
    ...
    def forward(self, input):
        weight = self.constval * self.alpha * F.layer_norm(self.weight, self.norm_shape, weight=None, bias=None, eps=self.gamma)
        if self.initalpha:
            self.initalpha = False
            out = F.linear(input, weight, self.bias)
            outvar = out.var()
            nn.init.constant_(self.alpha, torch.sqrt(2 / outvar))
            weight = self.constval * self.alpha * F.layer_norm(self.weight, self.norm_shape, weight=None, bias=None, eps=self.gamma)

        return F.linear(input, weight, self.bias)

    def extra_repr(self) -> str:
        return 'in_features={}, out_features={}, bias={}, gamma={}, initalpha={}'.format(
            self.in_features, self.out_features, self.bias is not None, self.gamma, self.initalpha
        )